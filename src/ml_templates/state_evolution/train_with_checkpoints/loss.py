...
class CrossEntropyLoss(AbstractLoss):
    ...
    def __init__(self, **kwargs):
        ...
        self.compute_loss = compute_cross_entropy_loss
...
def compute_cross_entropy_loss(model, x, y):
    _pred_y = vmap(model)(x)
    pred_y = jnp.take_along_axis(_pred_y, jnp.expand_dims(y, 1), axis=1)
